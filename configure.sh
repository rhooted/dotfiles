...
echo ''
read -p "Do you want to use rhooted's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down rhooted dotfiles..."
	git clone https://github.com/rhooted/dotfiles.git ~/.dotfiles
	echo ''
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with rhooted's macOS dotfiles..."
    else
        echo "rhooted's macOS dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply rhooted's macOS dotfiles. You will need to configure your environment manually..."
	echo ''
